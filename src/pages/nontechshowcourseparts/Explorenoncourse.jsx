...
const ExplorenontechCourse = () => {
	...
	const [SubTopicnameid, setSelectedSubtopicid] = useState(
		field.subtopic[0]._id
	);
	...
	const handlesubtopicclick = async (NonTechSubTopicnameid) => {
		...
		try {
			const response = await axios.post(
				"https://opencoursebackend.onrender.com/nontech/getnontechsubtopic",
							...
							screenWidth > 768
								...
								: `w-[80%] h-full ${
										sidebarVisible
											? "translate-x-0"
											: "-translate-x-full"
								  } absolute left-0 top-0 z-10 bg-bg-dark`
								...
								courses.map(
									(
										...
										<div
											...
											<h3 className="  text-white pl-4  border-b-2 border-cyan-900 ">
												Contributed
												by
												:{" "}
												{
