...
function Admin() {
	...
	const fetchFields = async () => {
		try {
			...
			data.forEach((field) => {
				topicsMap[field.name] = field.subtopic.map(
					(sub) => ({
						name: sub.name,
	...
	const fetchvideos = async () => {
		try {
			const response = await axios.get(
				"https://opencoursebackend.onrender.com/user/getvideos",
				{
					headers: {
						Authorization: `Bearer ${token}`,
					},
	...
	const handleAddTopic = async () => {
		...
		if (
			!topics[selectedField.name]?.some(
			...
			try {
				...
				if (error.response) {
					alert(
						`Failed to add topic: ${error.response.data.error}`
					);
					...
					alert(
						"Failed to add topic due to network error"
					);
					toast.error(
						"Failed to add topic due to network error.",
						{
		...
		if (title && url && description && selectedTopic) {
			try {
				...
				if (error.response?.status === 403) {
					alert(
						"This subtopic already has videos from 5 different users."
					);
				} else {
					console.error(
			...
			toast.warn(
				"Please fill in all details and select a subtopic.",
				{
					position: "top-right",
					autoClose: 5000,
					hideProgressBar: false,
