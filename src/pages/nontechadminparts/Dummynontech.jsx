	...
	const fetchCourses = async () => {
		try {
			const response = await axios.get(
				"https://opencoursebackend.onrender.com/nontech/getnontechcourse"
			);
			...
			{!token ? (
				...
				<div className="flex flex-col   justify-center items-center mt-20  ">
					...
					<form
						...
						className="  w-[75%] bg-gray-800 border rounded-lg  shadow-md">
						<div className=" flex flex-row p-2 pl-3   justify-between items-center  w-full   rounded-lg focus:outline-none focus:ring-2">
							<h2 className=" text-center text-xl text-white font-light  mb-2">
								Initialize
								course & Start
								contributing
							</h2>
							<CiCirclePlus
								className=" hover:text-yellow-400 mr-4 text-2xl text-white"
								onClick={() => {
									setseefullform(
										!seefullform
									);
